  ...
  (test-suite "Building Build SS bridge"
    ...
    (test-case "publishes one explicit ownership contract"
      (let (contract (framework-build-contract))
        ...
        (check (cdr (assoc 'generatedArtifactCacheOwner contract))
               => "ASP-verified-content-addressed-sidecar")
        (check (cdr (assoc 'generatedArtifactReuseAuthority contract))
               => "receipt-plus-byte-digest-plus-domain-validator")
        (check (cdr (assoc 'generatedArtifactNativeFreshnessOwner contract))
               => "std/make")
        (check (cdr (assoc 'generatedModuleProjection contract))
               => "native-gxc-extra-inputs")
